    ...
    if 'uid' in session:
        ...
        if get_result > 0:
            ...
            if request.method == 'POST' and form.validate():
                ...
                b=cur.execute("SELECT users.name FROM users INNER JOIN messages ON users.id = messages.msg_to WHERE users.id=%s",[uid])
                mysql.connection.commit()
                text=cur.fetchall()

                c=(text[0]["name"])

                # to display notification in desktop 

                notification.notify(
                title="CHAT APP MESSAGE FROM {}".format(c),
                message=txt_body,
                timeout=15
                )
            # Get users

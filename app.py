...
app.config['MYSQL_DB'] = 'vicky'
app.config['MYSQL_PORT']=3306

app.config['MYSQL_CURSORCLASS'] = 'DictCursor'
#app.config['MYSQL_OPTIONS']="SET GLOBAL max_connections = 100000"
    ...
    if 'uid' in session:
        ...
        if get_result > 0:
            ...
            if request.method == 'POST' and form.validate():
                ...
                # Create cursor
                cur=mysql.connection.cursor()
                cur.execute("INSERT INTO messages(body, msg_by, msg_to) VALUES(aes_encrypt(%s,'key1234'), %s, %s)",
                            (txt_body, id, uid))
                # Commit cursor
                mysql.connection.commit()     
...
def chats():
    if 'lid' in session:
        ...
        cur = mysql.connection.cursor()
        # Get message
        cur.execute("SELECT id,(cast(aes_decrypt(body,'key1234') as char)) as body,msg_by,msg_to,msg_time FROM messages WHERE (msg_by=%s AND msg_to=%s) OR (msg_by=%s AND msg_to=%s) "
                    "ORDER BY id DESC", (id, uid, uid, id))
        chats = cur.fetchall()
        # Close Connection
        cur.close()
...
""" CREATE TABLES MESSAGES AND USERS IN YOUR DATABASE LIKE THIS
